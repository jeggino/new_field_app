...
def get_device_fingerprint():
    """Generate a unique fingerprint for this browser/device session."""
    # Use a combination of user agent and a random component stored in query params
    user_agent = st.query_params.get("ua", [""])[0] if hasattr(st, 'query_params') else ""
    
    # If no fingerprint in URL, generate one and redirect
    fp = st.query_params.get("fp", [""])[0] if hasattr(st, 'query_params') else ""
    
    if not fp:
        fp = hashlib.sha256(f"{user_agent}{uuid.uuid4().hex}".encode()).hexdigest()[:16]
        # Store in query params (persists across reruns in same tab)
        st.query_params["fp"] = fp
        st.rerun()
...
class DeviceSessionManager:
    """
    Manages sessions per device using a combination of:
    1. Browser fingerprint (query param)
    2. Supabase session validation
    3. Server-side session state keyed by device fingerprint
    """
    
    def __init__(self):
        self.fp = get_device_fingerprint()
        self.session_key = f"device_session_{self.fp}"
        self._init_state()
    
    def _init_state(self):
        """Initialize device-specific session state."""
        # Only initialize defaults if this device hasn't been seen before
    ...
    def set(self, key, value):
        ...
        st.session_state[self.session_key]["last_activity"] = datetime.utcnow().isoformat()
    ...
    def is_logged_in(self):
        return self.get("logged_in") and self.get("user") is not None
    
    def validate_session(self, supabase_client):
        """
        CRITICAL: Verify the Supabase session is still valid for this user.
        This prevents session hijacking between devices.
        """
        ...
        try:
            # Verify with Supabase that this session is still valid
            current_session = supabase_client.auth.get_session()
            if current_session and current_session.user:
                # Check if the stored user matches the current session user
                stored_user_id = self.get("user", {}).get("id") if self.get("user") else None
                current_user_id = current_session.user.id
                
                if stored_user_id == current_user_id:
                    # Update session data
                    self.set("session", current_session)
                    self.set("user", current_session.user)
                    return True
...
device_session = DeviceSessionManager()
...
def logout():
    try:
        ...
        pass
    device_session.clear()
    st.rerun()

# ----------------- DATA HELPERS -----------------
def load_projects():
    user = device_session.get("user")
    if not user:
        return []
    res = (
...
def load_observations(project_name: str):
    ...
    device_session.set("observations", res.data or [])
    
    if res.data:
        last = res.data[-1]
        device_session.set("map_center", [last["lat"], last["lon"]])
        device_session.set("map_input_center", [last["lat"], last["lon"]])
...
def download_observations_csv():
    obs = device_session.get("observations", [])
    if not obs:
        return None
    df = pd.DataFrame(obs)
    return df.to_csv(index=False).encode("utf-8")
...
def daily_report_dialog():
    ...
    start_time = st.time_input("Start Time")
    end_time = st.time_input("End Time")
    operator = st.text_input("Operator", value=device_session.get("user", {}).get("email", ""))
    extra_operator = st.text_input("Extra Operator")
    temperature = st.number_input("Temperature (°C)", step=1)
    wind = st.number_input("Wind", step=1)
    rain = st.selectbox("Rain", REPORT_RAIN)
    ...
    if st.button("Submit Report", use_container_width=True):
        existing = (
            supabase.table("report")
            .select("id")
            .eq("project", device_session.get("project"))
            .eq("kind", kind)
            .execute()
        )
        if existing.data:
        ...
        supabase.table("report").insert({
            ...
            "rain": rain,
            "comment": comment,
            "project": device_session.get("project")
        }).execute()
...
def edit_observation_dialog(obs):
    ...
    if st.button("Update", use_container_width=True):
        ...
        load_observations(device_session.get("project"))
        st.rerun()
...
def new_observation_dialog():
    st.write("Use the map center as the observation position.")
    base_center = device_session.get("map_input_center")
    zoom = 20
    ...
    aantal = st.number_input("amount", step=1, value=1)
    behavior = st.text_area("Comments")
    username = device_session.get("user", {}).get("email", "")
    photo = st.file_uploader("Photo (optional)", type=["jpg", "jpeg", "png"])
    ...
    if st.button("Save observation", use_container_width=True):
        ...
        supabase.table(OBS_TABLE).insert(data).execute()
        device_session.set("map_center", [float(lat), float(lon)])
        device_session.set("map_input_center", [float(lat), float(lon)])
        load_observations(device_session.get("project"))
        st.rerun()
...
def show_login():
    ...
    with st.sidebar.form("login_form"):
        ...
        if submitted:
            res = login(email, password)
            if res and res.user:
                # Store in device-specific session, NOT global
                device_session.set("logged_in", True)
                device_session.set("user", res.user)
                device_session.set("session", res.session)
                st.rerun()
            else:
                st.sidebar.error("Invalid email or password")
    
    if st.sidebar.button("Create Account"):
        device_session.set("show_signup", True)
        st.rerun()
...
def show_signup():
    ...
    with st.sidebar.form("signup_form"):
        ...
        if submitted:
            ...
            if res and res.user:
                st.sidebar.success("Account created. Please log in.")
                device_session.set("show_signup", False)
                st.rerun()
            else:
                st.sidebar.error("Sign-up failed")
...
def show_project_selection():
    ...
    res = (
        supabase.table("project_members")
        .select("project")
        .eq("user_id", device_session.get("user").id)
        .execute()
    )
    rows = res.data or []
    ...
    if st.sidebar.button("Confirm project", use_container_width=True):
        device_session.set("project", selected)
        supabase.auth.update_user({"data": {"project": selected}})
        load_observations(selected)
        device_session.set("changing_project", False)
        st.rerun()
...
def show_main_app():
    ...
    if st.sidebar.button("Change Project", use_container_width=True, icon=":material/sync_alt:"):
        device_session.set("changing_project", True)
        st.rerun()
    ...
    prev_species = device_session.get("filter_species", [])
    prev_functions = device_session.get("filter_functions", [])
    ...
    selected_species = st.sidebar.multiselect(
        "Species", species_options, default=prev_species, key="filter_species"
    )
    selected_functions = st.sidebar.multiselect(
        "Function", function_options, default=prev_functions, key="filter_functions"
    )
    ...
    # MAP
    m = folium.Map(location=device_session.get("map_center"), zoom_start=12, zoom_control=False)
    LocateControl(auto_start=False).add_to(m)
    
    boundary, bounds = load_project_boundary(device_session.get("project"))
    if boundary:
        folium.GeoJson(
            boundary,
            name="Boundary",
    ...
    device_session.set("map_input_center", _get_center_from_map_data(map_data, device_session.get("map_center")))
    
    if map_data and map_data.get("last_object_clicked_popup"):
        obs_id = map_data.get("last_object_clicked_tooltip")
        if obs_id:
            device_session.set("selected_obs_id", obs_id)
    
    st.sidebar.divider()
    st.sidebar.header("Edit/Delete observation")
    selected_id = device_session.get("selected_obs_id")
    selected_obs = None
    ...
    if selected_obs:
        obs_id = str(selected_obs["id"])
        base_label = f"({obs_id}) {selected_obs.get('species','')} – {selected_obs.get('function','')}"
        if st.sidebar.button(base_label, key=f"obs_{obs_id}", use_container_width=True):
            edit_observation_dialog(selected_obs)
...
def restore_session_after_functions():
    ...
    This prevents session bleeding between different users/devices.
    """
    # First, validate the device-specific session
    if device_session.is_logged_in():
        # Verify with Supabase that this session is still valid
        if device_session.validate_session(supabase):
            return  # Valid session for this device, do nothing
    
    # If we get here, either no session or invalid session
    # Check if Supabase has a session (might be from another device)
    try:
        sess = supabase.auth.get_session()
        if sess and sess.user:
            # Supabase has a session, but it's NOT for this device
            # DO NOT auto-login. Force explicit login.
            supabase.auth.sign_out()
    except Exception:
        pass
    
    # Ensure device session is clear
    if not device_session.is_logged_in():
        device_session.clear()
...
def main():
    if not device_session.is_logged_in():
        if device_session.get("show_signup"):
            show_signup()
        else:
            show_login()
    elif device_session.get("changing_project"):
        show_project_selection()
        st.sidebar.divider()
        if st.sidebar.button("Logout", use_container_width=True, icon=":material/login:"):
            logout()
    elif not device_session.get("project"):
        show_project_selection()
        st.sidebar.divider()
        if st.sidebar.button("Logout", use_container_width=True, icon=":material/login:"):
            logout()
