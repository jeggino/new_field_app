...
# ---------------------------------------------------------
    ...
    with st.expander("Projectoverzicht", expanded=False, type="step",icon=":material/bar_chart:"):
...
#------------------------
    ...
    folium.TileLayer(tiles=tiles,
        ...
        max_native_zoom=22,
        overlay=False,
        control=False,name="Satellietkaart").add_to(m_html)
    ...
    # Load Font Awesome
    m_html.get_root().header.add_child(
        folium.Element(
            '<link rel="stylesheet" '
            'href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">'
        )
    ...
    m_html.get_root().add_child(legend)
    ...
    m_html.get_root().html.add_child(
        Element(css)
    )
    ...
    m_html.get_root().html.add_child(Element(logo_html))
...
# --------------------------------------------------
    ...
    if upload_response.status_code in [200, 201]:
        ...
        st.subhelder(f"De kaart is met succes naar de GitHub‑repository geüpload. Klik op deze [:red[**link**]]({github_url}) om de kaart te bekijken en te delen als je dat wilt.")
